            ...
            <div class="profile-info-summery">
                <div class="profile-info-head">
                    ...
                    <?php if ( ! empty( $store_user->get_shop_name() ) && 'default' === $profile_layout ) { ?>
                        <h1 class="store-name"><?php echo esc_html( $nickname ); ?></h1>
                    <?php } ?>
                   
                </div>

                <div class="profile-info">
                    <?php if ( ! empty( $nickname ) && 'default' !== $profile_layout ) { ?>
                        <h1 class="store-name"><?php echo esc_html( $nickname ); ?></h1>
                    <?php } ?>
                    ...
                    <ul class="dokan-store-info">
						...
						if ( isset( $store_info['address'] ) && !empty( $store_info['address'] ) ) { 
							$address_details_seller = $store_info['address'];
							$city = $address_details_seller['city'];
							$zip = $address_details_seller['zip'];

                             echo '<li class="dokan-store-address"><a target="_blank" href="https://www.google.fr/maps/place/' . $city . '/" title="Où se trouve ' . $city . ' "><i class="fa fa-map-marker"></i>' . $zip .' '. $city . '</a>
						...
						?>
                            ...
                            <li class="dokan-store-address">
								...
								<?php echo htmlspecialchars( date( "m/Y", strtotime( $registered ) ));?>
