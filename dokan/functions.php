...
function dokan_delete_product_handler() {
    if ( isset( $_GET['action'] ) && $_GET['action'] == 'dokan-delete-product' ) {
        ...
        trash_product( $product_id ); 

        do_action( 'dokan_product_deleted', $product_id );
		
		$staff_id  = get_current_user_id();
		$vendor_id = get_user_meta( $staff_id );
		$vendor_name =	$vendor_id ['nickname'][0];
		
		$subject_mail = 'Suppression annonce numéro: ' . $product_id . ' de ' . $vendor_name;
		$message_mail = 'L\'utilisateur ' . $vendor_name . ' a demandé la suppression de son article numéro: ' . $product_id;
		
		wp_mail( 'niels.abadie@luzus.fr', $subject_mail, $message_mail ); 

        $redirect = apply_filters( 'dokan_add_new_product_redirect', dokan_get_navigation_url( 'products' ), '' );

        wp_redirect( add_query_arg( array( 'message' => 'Votre trésor a bien été mis dans notre to do list "à supprimer". Nous vérifions qu\'aucune transaction n\'est en cours. La suppression sera effective dans les 24/48h. '),  $redirect ) );
